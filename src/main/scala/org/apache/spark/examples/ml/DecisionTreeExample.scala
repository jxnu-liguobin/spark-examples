 ...
 *  }}}
 *  Note that Decision Trees can take a large amount of memory. If the run-example command above fails, try running via
 *  spark-submit and specifying the amount of memory as at least 1g. For local mode, run
 *  {{{
...
object DecisionTreeExample {
  ...
  private[ml] def loadData(
    ...
    path: String,
    format: String,
    expectedNumFeatures: Option[Int] = None
  ): DataFrame = {
   ...
   *  @param fracTest
   *    Fraction of input data to hold out for testing. Ignored if testInput given.
   *  @return
   ...
   */
  private[ml] def evaluateClassificationModel(model: Transformer, data: DataFrame, labelColName: String): Unit = {
    ...
    val numClasses = MetadataUtils.getNumClasses(fullPredictions.schema(labelColName)) match {
      case Some(n) => n
      case None    => throw new RuntimeException("Unknown failure when indexing labels for classification.")
    }
   ...
   */
  private[ml] def evaluateRegressionModel(model: Transformer, data: DataFrame, labelColName: String): Unit = {
    val fullPredictions = model.transform(data).cache()
    val predictions     = fullPredictions.select("prediction").rdd.map(_.getDouble(0))
    val labels          = fullPredictions.select(labelColName).rdd.map(_.getDouble(0))
